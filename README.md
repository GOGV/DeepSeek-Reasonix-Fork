...
- [`78e1ac4`](https://github.com/GOGV/DeepSeek-Reasonix-Fork/commit/78e1ac4) (2026-08-03) — A missing or unreadable `system_prompt_file` no longer aborts startup; Reasonix warns and falls back to the inline/default system prompt. (upstream [PR #7330](https://github.com/esengine/DeepSeek-Reasonix/pull/7330))
- [`9da8303`](https://github.com/GOGV/DeepSeek-Reasonix-Fork/commit/9da8303) (2026-08-03) — A relative `system_prompt_file` is now probed under the workspace root first (project override), then under the Reasonix home (global default); only when every location is missing does it fall back. (upstream [PR #7330](https://github.com/esengine/DeepSeek-Reasonix/pull/7330))
- [`d0501e0`](https://github.com/GOGV/DeepSeek-Reasonix-Fork/commit/d0501e0) (2026-08-03) — When all probe locations miss, the warning names the configured value and lists every tried path ("not found at any of these locations: ..."). (upstream [PR #7330](https://github.com/esengine/DeepSeek-Reasonix/pull/7330))
...
- [`88867aa`](https://github.com/GOGV/DeepSeek-Reasonix-Fork/commit/88867aa) (2026-08-03) — Session-title generation no longer sends `MaxTokens: 20` to reasoning models: DeepSeek v4 spends the whole budget on thinking and returns an empty title, so the title cache never filled and `GET /sessions` re-ran ~20 LLM calls on every poll (~28s). (upstream [PR #7329](https://github.com/esengine/DeepSeek-Reasonix/pull/7329))
- [`61fa2c9`](https://github.com/GOGV/DeepSeek-Reasonix-Fork/commit/61fa2c9) (2026-08-03) — Session titles are sticky per session file instead of mtime-invalidated: while a session was active, every sidebar poll regenerated its title (one LLM call each). (upstream [PR #7329](https://github.com/esengine/DeepSeek-Reasonix/pull/7329))
- [`e4482d8`](https://github.com/GOGV/DeepSeek-Reasonix-Fork/commit/e4482d86) (2026-08-03) — Session delete no longer fails silently: clicking × on the active session shows a localized "无法删除当前会话 / Cannot delete the active session" notice, and any other delete error surfaces the server's message instead of looking like a no-op. (upstream [PR #7334](https://github.com/esengine/DeepSeek-Reasonix/pull/7334))
