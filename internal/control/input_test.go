...
func TestGoalAutoResearchTriggersForLongHorizonGoals(t *testing.T) {
	...
	if c.GoalRuntime().TurnsLimit != 0 {
		t.Fatalf("research Goal should be unlimited: %+v", c.GoalRuntime())
	}
}
...
func TestGoalAutoResearchCanBeForcedOrDisabled(t *testing.T) {
	...
	if got := c.Compose("start"); strings.Contains(strings.ToLower(got), "autoresearch") || c.GoalRuntime().TurnsLimit != 0 {
		t.Fatalf("forced research compatibility flag must remain unlimited: %q %+v", got, c.GoalRuntime())
	}

	c.SetGoalWithResearchMode("持续排查这个线上卡顿直到根因明确", GoalResearchOff)
	if got := c.Compose("start"); strings.Contains(strings.ToLower(got), "autoresearch") || c.GoalRuntime().TurnsLimit != 0 {
		t.Fatalf("simple compatibility flag must remain unlimited: %q %+v", got, c.GoalRuntime())
	}
}
...
func TestGoalCommandPreservesResearchModeFlags(t *testing.T) {
	...
	if got := c.Compose("start"); strings.Contains(strings.ToLower(got), "autoresearch") || c.GoalRuntime().TurnsLimit != 0 {
		t.Fatalf("/goal --research must not select a turn quota: %q %+v", got, c.GoalRuntime())
	}
