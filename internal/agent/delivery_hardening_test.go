...
func TestRunSubAgentReviewReportExhaustionNamesRecovery(t *testing.T) {
	...
	if !IsReviewUnavailable(err) {
		// Accept either the Partial unavailable path or a still-clear
		// review_report failure message for older diagnostics.
		for _, want := range []string{"review_report", "Partial"} {
			if !strings.Contains(err.Error(), want) && !strings.Contains(err.Error(), "reviewer unavailable") {
				// continue checking
			}
		}
