...
func TestWebSlashRequestsFrontendHandoff(t *testing.T) {
	...
	if args := strings.Join(webHandoffArgs(m.launchWebResumePath, m.launchWebSessionID, m.launchWebModelRef, "balanced"), " "); strings.Contains(args, "--resume") || !strings.Contains(args, "--session-id "+wantID) {
...
func TestWebSlashResumesMaterializedSession(t *testing.T) {
	...
	if args := strings.Join(webHandoffArgs(m.launchWebResumePath, m.launchWebSessionID, m.launchWebModelRef, "balanced"), " "); !strings.Contains(args, "--resume "+path) || strings.Contains(args, "--session-id") {
