...
// preserves the existing token-mode callback contract.
{
  ...
  eq(profileTrigger.getAttribute("aria-label"), "Role setting · Balanced", "role setting trigger keeps its full accessible name");
  ok(profileTrigger.querySelector(".lucide-equal") !== null, "balanced work mode uses a simple equal icon");
  await act(async () => {
    profileTrigger.focus();
    await flushTimers();
  });
  eq(document.querySelector('[role="tooltip"]')?.textContent, "Role setting · Balanced: Auto planning, risk-tiered verification", "role setting tooltip combines category, value, and summary");
  await act(async () => {
    profileTrigger.blur();
    await flushTimers();
  });
